...
for row in range(0, I):
    for col in range(0, J):
        ...
        #xi = magnitude[0][row]
        #yi = magnitude[col][0]
        for xi in range (0, matriceCopieSuperieur.shape[0]):
            for yi in range(0, matriceCopieSuperieur.shape[1]):
                if matriceCopieSuperieur[xi][yi] > 0:
...
for x in range(selectedValues.shape[0]):
    for y in range(selectedValues.shape[1]):
        for z in range(selectedValues.shape[2]):
            ...
            cv2.circle(result, (a, b), rad, (0, 255, 0))
